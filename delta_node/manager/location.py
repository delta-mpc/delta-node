...
def task_cfg_file(task_id: int) -> str:
    dirname = os.path.join(config.storage_dir, "task", str(task_id))
...
def task_weight_file(task_id: int, round_id: int) -> str:
    dirname = os.path.join(config.storage_dir, "task", str(task_id), "weight")
    if not os.path.exists(dirname):
        os.makedirs(dirname, exist_ok=True)
...
def task_result_file(task_id: int, round_id: int, member_id: str = None) -> str:
    dirname = os.path.join(config.storage_dir, "task", str(task_id), "result")
    if not os.path.exists(dirname):
        os.makedirs(dirname, exist_ok=True)
