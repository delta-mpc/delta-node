...
class TaskManager(object):
    def __init__(self, task_id: int, *, session: Session = None) -> None:
        ...
        self._joined_members = [member.node_id for member in task.members if member.joined]
    ...
    def start_new_round(self, member_id: str, *, session: Session = None) -> int:
        last_round, status = get_member_round_status(self._task_id, member_id, session=session)
        with self._round_cond:
            if self._round_finished:
                if status == model.RoundStatus.RUNNING:
                    ...
                    member_finish_round(self._task_id, member_id, last_round, session=session)
                # new round
